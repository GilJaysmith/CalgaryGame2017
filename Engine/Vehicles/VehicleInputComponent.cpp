...
void VehicleInputComponent::OnUpdate(const Time& elapsed_time, UpdatePass::TYPE update_pass)
{
	if (update_pass == UpdatePass::BeforePhysics)
	{
		...
		if (m_InputHandler)
		{
			Message_VehicleSetInputs mvsi(m_InputHandler->Steer(), m_InputHandler->Acceleration(), m_InputHandler->Brake(), m_InputHandler->Handbrake(), m_InputHandler->Jump(), m_InputHandler->ResetOrientation());
